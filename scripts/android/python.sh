...
CC=$HOST-gcc CXX=$HOST-g++ AR=$HOST-ar RANLIB=$HOST-ranlib\
    ./configure --host=$HOST --build=$BUILD --prefix=$PREFIX --enable-static

make HOSTPYTHON=./hostpython HOSTPGEN=./Parser/hostpgen BLDSTATIC="$HOST-gcc -static"\
    CROSS_COMPILE=$HOST- CROSS_COMPILE_TARGET=yes HOSTARCH=$HOST BUILDARCH=$BUILD

make install HOSTPYTHON=./hostpython BLDSTATIC="$HOST-gcc -static"\
